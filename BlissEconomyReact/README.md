...
First, as the backend is still not implemented, you need to run a local server.
In order to run the json server locally use:

`json-server --watch db.json`
`json-server --port 4000 db.json`

After, use the following command to run the React app locally:
